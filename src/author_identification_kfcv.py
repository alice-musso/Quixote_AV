...
from data.features__ import *
from model import AuthorshipVerificator
import settings
from util.evaluation import f1_from_counters, kfcv, leave_one_out
import argparse
import pickle
import helpers
...
def main():
    ...
    for i, author in enumerate(args.authors):
        ...
        print(f'Validating the Verificator ({args.k}-FCV)')
        accuracy, f1, tp, fp, fn, tn, missclassified = kfcv(av, Xtr, ytr, files, groups, k=args.k, n_jobs=settings.N_JOBS)
    ...
    tee(f'{args.k}-FCV Macro-F1 = {macro_f1:.3f}', log)
    tee(f'{args.k}-FCV Micro-F1 = {micro_f1:.3f}', log)
    tee(f'{args.k}-FCV Accuracy = {acc_mean:.3f}', log)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--featsel', default=settings.GLOBAL_FEATSEL, metavar='FEAT_SEL_RATIO',
    ...
    parser.add_argument('--k', type=int, default=10, metavar='NFOLDS',
                        help=f"number of folds for cross validation")
