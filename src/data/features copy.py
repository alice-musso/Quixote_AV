...
class FeaturesDVEX:
    ...
    def distortion(self, docs):
        ...
        for doc in docs:
            ...
            tokens = [str(token) for token in preocessed_doc]
...
class FeaturesMendenhall:
    ...
    def transform(self, documents, y=None):
        ...
        for doc in documents:
            ...
            word_lengths = [len(str(token)) for token in processed_doc]
...
class FeaturesSentenceLength:
    ...
    def transform(self, documents, y=None):
        ...
        for doc in documents:
            preocessed_doc = NLP(doc)
            sentences = [str(sentence) for sentence in preocessed_doc.sents]
            #sentences = [t.strip() for t in nltk.tokenize.sent_tokenize(doc, language=self.language) if t.strip()]
            sentence_lengths = []
            for sentence in preocessed_doc.sents:
...
class FeaturesPOST:
    ...
    def post_analyzer(self, text):
        ...
        for sentence in processed_doc.sents:
            ...
            try:
                sentence_unigram_tags = [tag[0] if tag != '' else 'Unk' for _, tag in sentence_wordtags] #prima tag[0]
            except IndexError:
...
class FeaturesDEP:
    ...
    def dep_analyzer(self, text):
        ...
        for sentence in processed_doc.sents:
        #for sentence in nltk.tokenize.sent_tokenize(text):
            sentence_deps = self.get_dependencies(str(sentence))
