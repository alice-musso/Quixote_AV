...
from helpers__ import get_function_words, metric_scansion, dis_DVMA, dis_DVSA, \
    dis_DVL2


def _vector_dis(doc_train, doc_test):
    vectorizer = TfidfVectorizer(analyzer='word', ngram_range=(3, 3), min_df=5, sublinear_tf=True)
...
class FeaturesDVEX:
    ...
    def __init__(self, function_words, ngram_range=(3,3), min_df=5, sublinear_tf=True):
    ...
    def fit(self, documents):
        ...
        self.vectorizer = TfidfVectorizer(analyzer='word', ngram_range=self.ngram_range, min_df=self.min_df, sublinear_tf=self.sublinear_tf)
        self.vectorizer.fit(distortions)
        return self

    def transform(self, documents):
        distortions = self.distortion(documents)
        return self.vectorizer.transform(distortions)

    def fit_transform(self, documents):
        distortions = self.distortion(documents)
        self.vectorizer = TfidfVectorizer(analyzer='word', ngram_range=self.ngram_range, min_df=self.min_df, sublinear_tf=self.sublinear_tf)
        return self.vectorizer.fit_transform(distortions)
...
# extract the frequency of each function word used in the documents
