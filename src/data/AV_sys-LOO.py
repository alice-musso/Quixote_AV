...
TEST_SIZE = 0.3
SEGMENT_MIN_TOKEN_SIZE = 500
RANDOM_STATE = 42
PROCESSED = True # whether the linguistic features have been already extracted or not
...
def oversample_positive_class(X_dev, X_test, y_dev, y_test, method='SMOTE'):
    ...
    if method =='DRO':
        ...
        sm = BorderlineSMOTE( sampling_strategy=0.2,random_state=42)
...
def extract_feature_vectors(processed_docs_dev, processed_docs_test, processed_docs_test_frag, y_dev):    #(documents, authors, filenames, nlp, target):
    ...
    for vectorizer in vectorizers:
        ...
        reductor =  FeatureSetReductor(vectorizer, k_ratio=0.2)
...
def save_res(target_author, accuracy, f1, cf, model_name, doc_name, file_name='verifiers_res_LOO_no_syl_quant.csv'):
...
def build_model(target, save_results=True, oversample=False):
    ...
    for i, (doc, ylabel) in enumerate(zip(documents,y)):
        ...
        for model, model_name in models:
            ...
            if save_results:
                save_res(target, acc, f1, cf, model_name, groups_test[0][:-2])
